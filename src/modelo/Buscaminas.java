...
public class Buscaminas {
	...
	public void generarMinas() {
		...
		while (cont < cantidadMinas) {
			int x = rd.nextInt(casillas.length);
			int y = rd.nextInt(casillas[0].length);
	...
	public String mostrarTablero() {
		...
		for (int i = 0; i < casillas.length; i++) {
			for (int j = 0; j < casillas[i].length; j++) {
				m += casillas[i][j].mostrarValorCasilla() + " ";
			}
			m += "\n";
		}
	...
	public int cantidadMinasAlrededor(int i, int j) {
		...
		if (i != 0 && j != 0 && casillas[i - 1][j - 1].esMina()) {
			num++;
		}
		if (i != 0 && casillas[i-1][j].esMina()) {
			num++;
		}

		if (i != 0 && j != casillas[i].length-1 && casillas[i-1][j+1].esMina()) {
			num++;
		}
		
		if (j != 0 && casillas[i][j-1].esMina()) {
			num++;
		}
		
		if (j != casillas[i].length -1 && casillas[i][j+1].esMina()) {
			num++;
		}
		
		if (i != casillas.length -1 && j != 0 && casillas[i + 1][j - 1].esMina()) {
			num++;
		}
		
		if (i !=  casillas.length -1 && casillas[i+1][j].esMina()) {
			num++;
		}
		
		if (i !=  casillas.length -1 && j != casillas[i].length -1 && casillas[i+1][j+1].esMina()) {
			num++;
		}
	...
	public String darPista() {
		String msg = "no hay pistas disponibles"; boolean enco = false;
		
		for (int i = 0; i < casillas.length && !enco; i++) {
			for (int j = 0; j < casillas[i].length && !enco; j++) {
				if (!casillas[i][j].esMina() && cantidadMinasAlrededor(i,j) > 0 && !casillas[i][j].darSeleccionada()) {
					msg = "la pista esta en cordenadas (" + (i+1) + "," + (j+1) + ")";
					abrirCasilla(i, j);
					enco = true;
				}
			}
